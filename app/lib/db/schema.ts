import { pgTable, serial, text, integer, timestamp, json, uuid, pgEnum } from 'drizzle-orm/pg-core';
...
export const relationTypeEnum = pgEnum('relation_type', [
  'family', 'friend', 'enemy', 'lover', 'colleague', 'other'
]);
...
export const characterRelations = pgTable('character_relations', {
  id: serial('id').primaryKey(),
  workId: integer('work_id').references(() => works.id, { onDelete: 'cascade' }).notNull(),
  characterId1: integer('character_id_1').references(() => characters.id, { onDelete: 'cascade' }).notNull(),
  characterId2: integer('character_id_2').references(() => characters.id, { onDelete: 'cascade' }).notNull(),
...
export const characterRelationsRelations = relations(characterRelations, ({ one }) => ({
  work: one(works, {
